    ...
    for e in eps:
        ...
        for t in e:
            ...
            if random.random() < epsilon:
                ...
                maxQ = max(Qopt)
                best = [i for i, q in enumerate(Qopt) if q == maxQ]
                i = random.choice(best) if len(best) > 1 else Qopt.index(maxQ)
                a_ = actions[i]

            Q[(s, a)] += eta * (r + gamma * max(Q.get((s_, act), 0.0) for act in actions) - Q.get((s, a), 0.0))
            numUpdates += 1
...
def ValueIteration(mdp):
    ...
    def Q(state, action):
        dis = mdp.discount()
        return sum(prob * (reward + dis * V[newState]) for newState, prob, reward in targets(mdp, state, action))
    ...
    while True:
        ...
        for state in reachable_states(mdp):
            if mdp.isEnd(state):
                ...
                values = [(Q(state, action), action) for action in mdp.actions(state)]
                newV[state], pi[state] = max(values)
